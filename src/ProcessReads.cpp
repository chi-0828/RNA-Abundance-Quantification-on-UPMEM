...
template <typename T>
std::vector<std::vector<T>> & buffer_align(std::vector<std::vector<T>> &buf){
  for(int i = 0; i < buf.size(); i++)
    while(((buf[i].size() * sizeof(T)) %8) != 0){
      T tmp;
      buf[i].push_back(std::move(tmp));
    }
  return buf;
}

template <typename T>
std::vector<T> & buffer_align(std::vector<T> &buf){
  while(((buf.size() * sizeof(T)) %8) != 0){
    T tmp;
...
size_t size_align(size_t size){
  if(size%8 == 0)
    return size;
  int push_size = 8 - (size%8);
  return size + push_size;
}

void ReadProcessor::process_result(int dpu_id, int n, std::vector<std::vector<std::pair<int, int>>> &contig_ids,
                    std::vector<std::vector<int64_t>> &matched_id, std::vector<std::vector<int64_t>> &matched_pos, std::vector<std::vector<int>> &matched_contig_len)
{
  ...
  for(int read_id = 0; read_id < n; read_id++){
    ...
    /*int ec = -1;
    std::vector<int> dpu_classes = tc.DPU_intersectECs(tmp);
    read_count.first++;
    if (!dpu_classes.empty()) {
      ec = tc.findEC(dpu_classes);
      //assert(ec < 0 );
  ...
  for (int i = 0 ; ;){
    ...
    if(!remain_transfer){
      ...
      try {
        std::chrono::steady_clock::time_point begin = std::chrono::steady_clock::now();
        // tarnsfer reads
        dpu_set.copy("reads_len", buffer_align(dpu_buffer_len));
        dpu_set.copy("reads", buffer_align(dpu_buffer));
        std::vector<int32_t> read_n(1, packet_size);
        dpu_set.copy("read_n", read_n);
        std::chrono::steady_clock::time_point end = std::chrono::steady_clock::now();
        transfer_read_time += std::chrono::duration_cast<std::chrono::nanoseconds> (end - begin).count()  * 1e-9;
        ...
        dpu_set.copy(matched_id, "result_id");
        std::vector<std::vector<int64_t>> matched_pos(dpu_n, std::vector<int64_t>(max, -2));
        dpu_set.copy(matched_pos, "result_pos");
        end = std::chrono::steady_clock::now();
        transfer_result_time += std::chrono::duration_cast<std::chrono::nanoseconds> (end - begin).count()  * 1e-9;

        begin = std::chrono::steady_clock::now();
        // process the result
        for(int dpu_id = 0; dpu_id < dpu_n; dpu_id++){
          process_result(dpu_id, packet_size, contig_ids, matched_id, matched_pos, matched_contig_len);
        }
        end = std::chrono::steady_clock::now();
    ...
    if( i + (packet_size*dpu_n) >= seqs.size() || remain_transfer ){
      ...
      try {
        ...
        // process the result
        for(int dpu_id = 0; dpu_id < dpu_n; dpu_id++){
          process_result(dpu_id, read_n[dpu_id][0], contig_ids, matched_id, matched_pos, matched_contig_len);
        }
        end = std::chrono::steady_clock::now();
        process_time += std::chrono::duration_cast<std::chrono::nanoseconds> (end - begin).count()  * 1e-9;
...
void MasterProcessor::DPU_update(const std::vector<int>& c){
...
ReadProcessor::ReadProcessor(const KmerIndex& index, const ProgramOptions& opt, const MinCollector& tc, MasterProcessor& mp, int _id, int _local_id) :
 paired(!opt.single_end), tc(tc), index(index), mp(mp), id(_id), local_id(_local_id) {
   ...
   my_counts.reserve((int) (tc.counts.size() * 1.25));
   clear();
}
...
ReadProcessor::ReadProcessor(ReadProcessor && o) :
  ...
  my_counts(std::move(o.my_counts)) {
    transfer_read_time = 0;
    transfer_result_time = 0;
    DPU_run_time = 0;
    transfer_table_time = 0;
...
void ReadProcessor::operator()() {
  ...
  int nrthread = 1;
  for(int thid = 0; thid < nrthread ; thid ++){
    vecOfThreads.push_back(std::thread(&ReadProcessor::CPU_intsersection, this));
  }
...
void ReadProcessor::processBuffer(DpuSet &dpu_set) {

  std::vector<std::vector<std::pair<int, int>>> contig_ids;

  // start DPU process
  
  transfer_read(contig_ids, dpu_set);
  
  
  // check the DPU result
  // assert(seqs.size() == contig_ids.size());
}
...
void ReadProcessor::clear() {
  ...
  my_counts.clear();
  my_counts.resize(tc.counts.size(),0);
  ec_umi.clear();
  new_ec_umi.clear();
}
