...
int64_t result_id_tasklet[NR_TASKLETS][(MAX_PACKET_SIZE/NR_TASKLETS) + MAX_READ_N];
int64_t result_pos_tasklet[NR_TASKLETS][(MAX_PACKET_SIZE/NR_TASKLETS) + MAX_READ_N];
int matched_size_tasklet[NR_TASKLETS];
...
int main(){
  ...
  if(tasklet_id == 0){
    ...
    matched_size = 0;
    memset(result_id_tasklet, -2, MAX_PACKET_SIZE);
    memset(result_pos_tasklet, -2, MAX_PACKET_SIZE);
    memset(matched_size_tasklet, -1, NR_TASKLETS);
  ...
  for(int readid = read_start ; readid < new_read_n; readid++){
    ...
    int matched_count_read = v_matched;

    // iterate each read
    v_matched = match(read_cache, shift_count, len, &kmap, vint, vpos, v_matched, result_id, result_pos);

    // get matched count of this read
    matched_count_read = (v_matched - matched_count_read);
